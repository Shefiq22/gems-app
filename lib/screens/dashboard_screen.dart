// ============================================================
//  GEMS — Dashboard Screen (role-aware, all live data)
//
//  Admin        → full university overview, all faculties
//  Faculty Officer → their faculty only, pending veg reports
//  Groundskeeper   → their task list + submit report
//  Student         → read-only score view + report issue
// ============================================================

import 'package:flutter/material.dart';
import 'package:flutter_animate/flutter_animate.dart';
import 'package:gems/screens/module_screen.dart';
...
class _DashboardScreenState extends State<DashboardScreen>
  ...
  Future<void> _loadData() async {
    ...
    try {
      // Faculty officers only see their own faculty's data
      final facultyFilter =
          (_role == 'faculty_officer' && _facultyId.isNotEmpty)
              ? _facultyId
              : null;

      final rawFaculties = await SupabaseService.getFaculties();
      final rawTasks     = await SupabaseService.getTasks(
          facultyId: facultyFilter);
      final rawReports   = await SupabaseService.getMonthlyReports();
      ...
      // Attach tasks to matching faculties
      ...
      // Filter faculties for officer view
      final visibleFaculties = facultyFilter != null
          ? faculties.where((f) => f.id == facultyFilter).toList()
          : faculties;
  ...
  int get _criticalTasks => _tasks
      .where((t) => t.status == 'overdue' || t.priority == 'critical')
      .length;
  ...
  Widget _buildBody() {
    switch (_selectedNav) {
      ...
      case 8:  return const AlertsScreen();
      ...
      return Scaffold(
        ...
        body: Center(
          child: Column(
            ...
            children: [
              const CircularProgressIndicator(color: GEMSTheme.primaryGreen),
              const SizedBox(height: 20),
              Text('Loading GEMS data...',
        ...
        body: Center(
          child: Container(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              Text('Could not load data', style: GEMSTheme.headingMedium),
              const SizedBox(height: 8),
              Text(_error!, style: GEMSTheme.bodySmall,
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('Abiola Ajimobi University — GEMS',
                      style: GEMSTheme.bodySmall),
                  const SizedBox(height: 4),
                  Text('Campus Overview', style: GEMSTheme.displayMedium),
                ],
              ...
              Row(
                children: [
                  if (_error != null)
                    Tooltip(
                      message: 'Some data failed to load: $_error',
                      child: const Padding(
          ...
          Row(
            children: [
              Expanded(child: GEMSStatCard(
                title: 'Overall GHI Score',
              ...
              Expanded(child: GEMSStatCard(
                ...
                value: '47 ha',
                subtitle: '${University.ageYears} year old campus',
                icon: Icons.map_outlined,
                color: GEMSTheme.emerald,
                trend: '4 active faculty zones',
                positive: true,
            ...
            children: [
              Expanded(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('Faculty Zones', style: GEMSTheme.headingLarge),
                    const SizedBox(height: 4),
              ...
              Expanded(
                ...
                child: Column(
                  children: [
                    ...
                    const SizedBox(height: 20),
                    _RecentAlerts(tasks: _tasks, faculties: _faculties)
  ...
  Widget _buildFacultiesTab() {
    return SingleChildScrollView(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(_role == 'faculty_officer' ? 'My Faculty' : 'All Faculties',
          ...
          if (_faculties.isEmpty)
            Center(
              child: Text('No faculty data. Pull to refresh.',
                  style: GEMSTheme.bodySmall),
            )
          else
  ...
  Widget _buildTasksTab() {
    final canCreate = _role == 'admin' || _role == 'faculty_officer';
        ...
        children: [
          Row(
            ...
            children: [
              Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('Maintenance Tasks', style: GEMSTheme.displayMedium),
                  const SizedBox(height: 4),
                  Text('Scheduled and ongoing maintenance activities',
                      style: GEMSTheme.bodySmall),
                ],
              ),
              if (canCreate)
          ...
          if (_tasks.isEmpty)
            Center(
              ...
              child: Text('No tasks found.', style: GEMSTheme.bodySmall),
            ))
          else
            ..._tasks.asMap().entries.map((e) {
              ...
              final faculty = _faculties.firstWhere(
                ...
                orElse: () => Faculty(
                  ...
                  imageUrl: '', hazardLevel: 'low', totalArea: 0, dean: '',
                ),
              );
              return _TaskRow(faculty: faculty, task: task,
  ...
  void _showCreateTaskDialog() {
    ...
    String fId      = _faculties.isNotEmpty ? _faculties.first.id : 'nas';
        ...
        return AlertDialog(
          ...
          content: SizedBox(
            ...
            child: SingleChildScrollView(
              child: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  _DlgField('Task Title', titleCtrl, 'e.g. Bush Clearing — Block A'),
                  const SizedBox(height: 12),
                  _DlgField('Description', descCtrl, 'What needs to be done...', maxLines: 3),
                  const SizedBox(height: 12),
                  _DlgField('Assigned To', assignCtrl, 'e.g. Groundskeeping Team A'),
                  const SizedBox(height: 12),
                  Row(children: [
                    Expanded(child: _DlgDrop('Faculty', fId,
                        _faculties.map((f) => (f.id, f.shortName)).toList(),
                        (v) => setDlg(() => fId = v!))),
                    const SizedBox(width: 12),
                    Expanded(child: _DlgDrop('Priority', priority,
                        const [('low','Low'),('medium','Medium'),('high','High'),('critical','Critical')],
                        (v) => setDlg(() => priority = v!))),
                  ]),
                  const SizedBox(height: 12),
                  Row(children: [
                    Expanded(child: _DlgDrop('Task Type', taskType,
                        const [
                        ...
                        (v) => setDlg(() => taskType = v!))),
                    const SizedBox(width: 12),
                    Expanded(child: _DlgDrop('Status', status,
                      ...
                      child: Row(
                        children: [
                          const Icon(Icons.calendar_today_outlined,
                              size: 18, color: GEMSTheme.textLight),
          ...
          actions: [
            TextButton(
              ...
              child: Text('Cancel',
                  style: GoogleFonts.poppins(color: GEMSTheme.textMid)),
            ),
            ElevatedButton(
              onPressed: () async {
  ...
  Widget _buildReportsTab() {
    return SingleChildScrollView(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Environmental Reports', style: GEMSTheme.displayMedium)
              .animate().fadeIn(),
          const SizedBox(height: 8),
          Text('Green Health Index trends and task analytics',
              style: GEMSTheme.bodySmall)
              .animate().fadeIn(delay: 100.ms),
          const SizedBox(height: 28),
          if (_reports.isEmpty)
            Container(
              ...
              child: Center(
                child: Text(
                  'No monthly reports yet.\nAdd records to the monthly_reports table.',
                  textAlign: TextAlign.center,
                  style: GEMSTheme.bodySmall,
                ),
              ),
            ...
            _FullReportChart(reports: _reports).animate().fadeIn(delay: 200.ms),
            const SizedBox(height: 24),
            Row(
              children: [
                Expanded(
                    child: _VegetationBreakdown(faculties: _faculties)
...
class _RoleBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    switch (role) {
      case 'admin':
        msg   = 'Admin View — monitoring all 4 faculties across campus';
        color = GEMSTheme.primaryGreen;
        icon  = Icons.admin_panel_settings_rounded;
        break;
      case 'faculty_officer':
        msg   = 'Faculty Officer View — ${facultyId.toUpperCase()} zone only';
        color = GEMSTheme.emerald;
        icon  = Icons.account_balance_rounded;
        break;
      case 'groundskeeper':
        msg   = 'Groundskeeper View — submit field reports and view your tasks';
        color = GEMSTheme.warning;
        icon  = Icons.agriculture_rounded;
        break;
      default:
        msg   = 'Student View — read-only campus overview';
...
class _HeroBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final sorted = [...faculties]
      ..sort((a, b) => b.greenHealthScore.compareTo(a.greenHealthScore));
      ...
      builder: (_, __) => Container(
        decoration: BoxDecoration(
          ...
          gradient: LinearGradient(
            ...
            colors: [
              Color.lerp(GEMSTheme.primaryGreen, GEMSTheme.forestGreen,
                  controller.value)!,
              Color.lerp(GEMSTheme.forestGreen, GEMSTheme.emerald,
                  controller.value)!,
              Color.lerp(GEMSTheme.emerald, const Color(0xFF004D40),
                  controller.value)!,
            ],
          ),
                      ...
                      children: [
                        ...
                        Text(
                          best != null && worst != null
                              ? '${best.shortName} leads at ${best.greenHealthScore.toInt()}/100. '
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 4),
          Text('Green Health Index by Faculty', style: GEMSTheme.bodySmall),
          const SizedBox(height: 20),
          SizedBox(
            height: 160,
...
class _RecentAlerts extends StatelessWidget {
  ...
  final List<Faculty> faculties;
  const _RecentAlerts(
      {required this.tasks, required this.faculties});
...
// ══════════════════════════════════════════════════════════════
//  FACULTY LIST CARD (in Faculties tab)
// ══════════════════════════════════════════════════════════════
...
class _FacultyListCard extends StatefulWidget {
  ...
  @override
  State<_FacultyListCard> createState() => _FacultyListCardState();
}
    ...
    return MouseRegion(
      ...
      child: GestureDetector(
        onTap: () => Navigator.of(context).push(MaterialPageRoute(
            builder: (_) => FacultyDetailScreen(faculty: f))),
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [
              BoxShadow(
                color: f.color.withOpacity(_hovered ? 0.14 : 0.06),
          ...
          child: Row(
            children: [
              ...
              Icon(Icons.chevron_right,
                  color: _hovered ? f.color : GEMSTheme.textLight),
            ],
          ),
        ),
      ),
...
class _TaskRowState extends State<_TaskRow> {
  ...
  Widget build(BuildContext context) {
    ...
    return MouseRegion(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: _hovered ? Colors.white : const Color(0xFFF9FAF8),
          borderRadius: BorderRadius.circular(14),
          border: Border.all(
              color: _hovered ? f.color.withOpacity(0.3) : Colors.transparent),
          boxShadow: _hovered
              ? [BoxShadow(
        ...
        child: Row(
          children: [
            Container(
              width: 6, height: 48,
              decoration: BoxDecoration(
                  color: _sc, borderRadius: BorderRadius.circular(3)),
            ),
            ...
            const SizedBox(width: 6),
            _TBadge(
                t.status.replaceAll('_', ' ').toUpperCase(), _sc),
            const SizedBox(width: 10),
            // Quick status update
  ...
  PopupMenuItem<String> _mi(String v, String l, IconData i, Color c) =>
      PopupMenuItem(
        value: v,
        child: Row(children: [
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Green Health Index — Monthly Progress',
              style: GEMSTheme.headingLarge),
          const SizedBox(height: 4),
          Text('All faculties tracked monthly', style: GEMSTheme.bodySmall),
          const SizedBox(height: 24),
          SizedBox(
            height: 280,
...
class _VegetationBreakdown extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // Aggregate percentages from live zone data
    ...
    if (zoneCount == 0) {
      return Container(
        padding: const EdgeInsets.all(24),
        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20), boxShadow: [GEMSTheme.softShadow]),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Campus Vegetation Breakdown', style: GEMSTheme.headingMedium),
            const SizedBox(height: 16),
            Text('No zone data — add vegetation_zones records in Supabase.', style: GEMSTheme.bodySmall),
          ],
        ),
      );
    }
    final total = totals.values.fold(0.0, (a, b) => a + b);
    final items = totals.entries.map((e) => (e.key, e.value / total)).toList()
      ..sort((a, b) => b.$2.compareTo(a.$2));
    final colors = <String, Color>{
      'bush': const Color(0xFF4E342E), 'grass': GEMSTheme.accentGreen,
      'trees': GEMSTheme.forestGreen, 'bare': const Color(0xFFBCAAA4),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 20),
          ...items.map((item) {
            final c = colors[item.$1] ?? GEMSTheme.primaryGreen;
            return Padding(
              padding: const EdgeInsets.only(bottom: 14),
              child: Column(children: [
                Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  Text(item.$1.capitalize(), style: GEMSTheme.bodySmall),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Task Status Overview', style: GEMSTheme.headingMedium),
          const SizedBox(height: 20),
          SizedBox(
            height: 200,
...
class _DlgField extends StatelessWidget {
  ...
  final int maxLines;
  const _DlgField(this.label, this.ctrl, this.hint, {this.maxLines = 1});
  ...
  Widget build(BuildContext context) => Column(
        ...
        children: [
          ...
          TextField(
            controller: ctrl,
            maxLines: maxLines,
            style: GoogleFonts.poppins(fontSize: 13, color: GEMSTheme.textDark),
...
class _DlgDrop extends StatelessWidget {
  ...
  const _DlgDrop(this.label, this.value, this.items, this.onChange);
            ...
            child: DropdownButtonHideUnderline(
              child: DropdownButton<String>(
                ...
                items: items.map((i) => DropdownMenuItem(
                    value: i.$1, child: Text(i.$2))).toList(),
                onChanged: onChange,
              ),
            ),
          ),
          ...
          child: GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              child: Icon(widget.icon,
                  color: _h ? GEMSTheme.primaryGreen : GEMSTheme.textMid,
                  size: 20),
            ),
          ),
        ),
